...
def evaluate(distances, labels, nrof_folds=10):
    ...
    tpr, fpr, accuracy, tp, fp, fn = calculate_roc(thresholds, distances,
    ...
    return tpr, fpr, accuracy, val, val_std, far, tp, fp, fn
...
def calculate_roc(thresholds, distances, labels, nrof_folds=10):
    ...
    for fold_idx, (train_set, test_set) in enumerate(k_fold.split(indices)):
        ...
        for threshold_idx, threshold in enumerate(thresholds):
            _, _, acc_train[threshold_idx], tp, fp, fn = calculate_accuracy(threshold, distances[train_set], labels[train_set])
        best_threshold_index = np.argmax(acc_train)
        for threshold_idx, threshold in enumerate(thresholds):
            tprs[fold_idx, threshold_idx], fprs[fold_idx, threshold_idx], _, tp, fp, fn = calculate_accuracy(threshold,
                                                                                                 distances[test_set],
                                                                                                 labels[test_set])
        _, _, accuracy[fold_idx], tp, fp, fn = calculate_accuracy(thresholds[best_threshold_index], distances[test_set],
                                                      labels[test_set])

        tpr = np.mean(tprs, 0)
        fpr = np.mean(fprs, 0)
    return tpr, fpr, accuracy, tp, fp, fn
