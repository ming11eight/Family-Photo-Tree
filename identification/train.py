...
import wandb
# from inception_resnet_v1 import InceptionResnetV1import wandb
wandb.login()
...
parser.add_argument('--num-train-triplets', default=3000, type=int, metavar='NTT',
...
parser.add_argument('--step-size', default=1, type=int, metavar='SZ',
...
# main
def main():
    init_log_just_created("log/log2/valid.csv")
    init_log_just_created("log/log2/train.csv")
    import pandas as pd
    valid = pd.read_csv('log/log2/valid.csv')
    ...
    if args.load_best or args.load_last:
        checkpoint = './log/log2/best_state.pth' if args.load_best else './log/log2/last_checkpoint.pth'
...
def save_last_checkpoint(state):
    torch.save(state, 'log/log2/last_checkpoint.pth')
...
def train_valid(model, optimizer, triploss, scheduler, epoch, dataloaders, data_size):
    for phase in ['train', 'valid']:
        ...
        write_csv(f'log/log2/{phase}.csv', [time, epoch, np.mean(accuracy), avg_triplet_loss, layers, args.batch_size, lr])
        ...
        if phase == 'valid':
            ...
            if epoch==99:
                plot_roc(fpr, tpr, figure_name='./log/log2/roc_valid_epoch_{}.png'.format(epoch))
